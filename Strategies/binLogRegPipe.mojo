...
class BinLogReg_Pipeline:
	def __init__(self, data: pd.DataFrame, parameters: Dict[str, Any]) -> None:
		# Set parameters
	...
	def get_features(self, data_sample: pd.DataFrame) -> pd.DataFrame:
	...
	def get_entry_signal(self, time: pd.Timestamp) -> Tuple[int, pd.Timestamp]:
		"""
		Random Entry signal
		:param i: row number
	...
	def get_exit_signal(self, time: pd.Timestamp) -> Tuple[float, pd.Timestamp]:
		"""
