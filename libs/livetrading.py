...
class LiveTrading():
	def __init__(self, broker="schwab"):
		...
		self.qt = quotes2.Quotes(self.broker_name, log=None)
		self.account_hash = self.qt.account_hash
		self.broker = self.qt.broker
		self.positions = positions.Positions(self.broker, None)
	...
	def send_order(self, order):
		...
		que.send_msg(queue_name, json_message)
		return True
